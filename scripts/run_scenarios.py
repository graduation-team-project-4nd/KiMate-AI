...
def run_final_demo(client: TestClient) -> None:
    ...
    screens = [
        ...
        },
        {
            ...
            "user_inputs": ["에스프레소 없어?", "그럼 코카콜라-미디엄으로 줘"],
        },
        {
            "name": "화면10-요약",
            "texts": [
    ...
    for idx, step in enumerate(screens, 1):
        ...
        user_inputs = step["user_inputs"]
        primary_input = user_inputs[0] if user_inputs else None

        detect_resp = call_screen_detect(
            client=client,
            session_id=session_id,
            previous_texts=prev_texts,
            current_texts=step["texts"],
            dialogue_history=dialogue_history,
        ...
        if detect_resp.get("ai_analysis"):
            ai = detect_resp["ai_analysis"]
            append_turn(dialogue_history, "assistant", ai["response_message"], ai["action"])
            if ai["action"]["type"] == "click_text":
                last_btn = ai["action"]["params"].get("target_text", last_btn)
            print_resp("screen/detect→ai", ai)
        else:
            print(f"screen changed? {detect_resp['is_changed']} score={detect_resp['similarity_score']:.2f} (ai_analysis=None)")

        # Additional user inputs on the same screen (e.g., 되묻기 이후 답변)
        extra_inputs = user_inputs[1:] if user_inputs else []
